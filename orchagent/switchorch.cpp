...
void SwitchOrch::setFastLinkupCapability()
{
    ...
    // Determine support by checking create/set capability on polling time attribute
    ...
    // Query allowed ranges if supported by SAI
    {
        ...
        {
            SWSS_LOG_INFO("FAST_LINKUP polling range not supported: %s", sai_serialize_status(status).c_str());
        }
    }

    {
        ...
        {
            SWSS_LOG_INFO("FAST_LINKUP guard range not supported: %s", sai_serialize_status(status).c_str());
        }
    }
...
bool SwitchOrch::setSwitchFastLinkup(const FastLinkupConfig &cfg)
{
    ...
    {
        SWSS_LOG_ERROR("Fast link-up is not supported on this platform");
        return false;
    }
    ...
    if (cfg.has_polling && m_fastLinkupCap.has_ranges)
    {
        ...
        {
            SWSS_LOG_ERROR("Invalid polling_time %u; allowed [%u,%u]", cfg.polling_time, m_fastLinkupCap.polling_min, m_fastLinkupCap.polling_max);
            return false;
        }
    }
    if (cfg.has_guard && m_fastLinkupCap.has_ranges)
    {
        ...
        {
            SWSS_LOG_ERROR("Invalid guard_time %u; allowed [%u,%u]", cfg.guard_time, m_fastLinkupCap.guard_min, m_fastLinkupCap.guard_max);
            return false;
        }
    }
    ...
    if (cfg.has_polling)
    {
        sai_attribute_t attr; attr.id = SAI_SWITCH_ATTR_FAST_LINKUP_POLLING_TIME; attr.value.u16 = cfg.polling_time;
        status = sai_switch_api->set_switch_attribute(gSwitchId, &attr);
        if (status != SAI_STATUS_SUCCESS)
        {
            SWSS_LOG_ERROR("Failed to set FAST_LINKUP_POLLING_TIME=%u: %s", cfg.polling_time, sai_serialize_status(status).c_str());
            return false;
        }
    }
    ...
    if (cfg.has_guard)
    {
        sai_attribute_t attr; attr.id = SAI_SWITCH_ATTR_FAST_LINKUP_GUARD_TIME; attr.value.u8 = cfg.guard_time;
        status = sai_switch_api->set_switch_attribute(gSwitchId, &attr);
        if (status != SAI_STATUS_SUCCESS)
        {
            SWSS_LOG_ERROR("Failed to set FAST_LINKUP_GUARD_TIME=%u: %s", cfg.guard_time, sai_serialize_status(status).c_str());
            return false;
        }
    }
    ...
    if (cfg.has_ber)
    {
        sai_attribute_t attr; attr.id = SAI_SWITCH_ATTR_FAST_LINKUP_BER_THRESHOLD; attr.value.s8 = cfg.ber_threshold;
        status = sai_switch_api->set_switch_attribute(gSwitchId, &attr);
        if (status != SAI_STATUS_SUCCESS)
        {
            SWSS_LOG_ERROR("Failed to set FAST_LINKUP_BER_THRESHOLD=%d: %s", cfg.ber_threshold, sai_serialize_status(status).c_str());
            return false;
        }
    }

    SWSS_LOG_NOTICE("Configured fast link-up: polling%s guard%s ber%s",
