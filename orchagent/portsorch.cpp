...
task_process_status PortsOrch::setPortFastLinkupEnabled(Port &port, bool enable)
{
    ...
    {
        SWSS_LOG_NOTICE("Fast link-up per-port attribute not supported; skipping for %s", port.m_alias.c_str());
        return task_success;
    }
    ...
    SWSS_LOG_NOTICE("Set port %s fast_linkup %s", port.m_alias.c_str(), enable ? "true" : "false");
    return task_success;
}
...
void PortsOrch::generatePriorityGroupMapPerPort(const Port& port, FlexCounterPgStates& pgsState)
{
    ...
    m_pgCounterNameMapUpdater->setCounterNameMap(pgVector);
